/****************************************************************************
	LEAFLET-PRINT-RECTANGLE.JS, Leaflet-object to mark a fixed-ratio area on the map witch is to be printed

	(c) 2015, Niels Holt

	https://github.com/NielsHolt/leaflet-print-rectangle
...
When the rectangle is being dragged the marker on the other side of the rectangle relative to the marker 
being dragged is fixed and the new size is calculated acording to this fixed point
	...
	L.PrintRectangle = L.Class.extend({
		...
		**************************************************************************************/
		addTo: function(map) {
			...
			this.rectangle = new L.Rectangle(map.getBounds(), lprPath);
			...
			if (this.options.allowRotate)
				this.rectangle.on('click', this.rotate, this);
		...
		**************************************************************************************/
		_update: function(){
			...
			var bounds = L.latLngBounds(
				this.map.containerPointToLatLng( [this.left, this.bottom] ),	//southWest 
				this.map.containerPointToLatLng( [this.right, this.top] )			//northEast
			);
			...
			this.resizeMarkers[posLeft  + posTop		].setLatLng( bounds.getNorthWest() );
			this.resizeMarkers[posLeft	 + posBottom	].setLatLng( bounds.getSouthWest() );
			this.resizeMarkers[posRight + posTop		].setLatLng( bounds.getNorthEast() );
			this.resizeMarkers[posRight + posBottom	].setLatLng( bounds.getSouthEast() );
	
			//Move the resize marker on the four sides
			this.resizeMarkers[ posLeft		+ posMiddle ].setLatLng( this.map.containerPointToLatLng( [this.left,	 this.middle ] ) );
			this.resizeMarkers[ posRight		+ posMiddle ].setLatLng( this.map.containerPointToLatLng( [this.right,	 this.middle ] ) );
			this.resizeMarkers[ posCenter	+ posTop		 ].setLatLng( this.map.containerPointToLatLng( [this.center, this.top		 ] ) );
			this.resizeMarkers[ posCenter	+ posBottom ].setLatLng( this.map.containerPointToLatLng( [this.center, this.bottom ] ) );
	
			//Move the center/midle marker
			this.resizeMarkers[ posCenter	+ posMiddle ].setLatLng( this.map.containerPointToLatLng( [this.center, this.middle] ) );
		...
		**************************************************************************************/
		_calcPointBounds: function(){
			var NWPoint = this.map.latLngToContainerPoint( this.rectangle.getBounds().getNorthWest() );
			var SEPoint = this.map.latLngToContainerPoint( this.rectangle.getBounds().getSouthEast() );
			this.left = NWPoint.x; 
			this.top = NWPoint.y; 
		...
		**************************************************************************************/
		setRatio: function( ratio ){
			...
			if (!this.maxBounds.contains( this.rectangle.getBounds() ) ){
